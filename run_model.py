...
def run_model():
    ...
    if args.test_dataset:
        ...
        if "CoxnetSurvivalAnalysis" in args.pipe_steps[-1]:
            param_grid_dict = update_coxnet_param_grid_dict(search, param_grid_dict)
        ...
        if "Weight" in final_feature_meta.columns:
            ...
            if isinstance(best_pipe[-1], (RFE, SelectFromUnivariateSurvivalModel)):
    ...
    # train-test nested cv
        ...
        for split_idx, (train_idxs, test_idxs) in enumerate(
            ...
            try:
                ...
                risk_scores = (
                    risk_scores.join(split_risk_scores, how="outer")
                    if risk_scores is not None
                    else split_risk_scores
                )
        ...
        if param_grid_dict_alphas is not None:
            param_grid_dict[cnet_srv_a_param] = np.mean(param_grid_dict_alphas, axis=0)
        scores = {"cv": {}, "te": {}}
        num_features = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skb-slr-k-max", type=int, help="Selector k max features")
    parser.add_argument(
        "--skb-slr-k-step", type=int, default=1, help="Selector k step features"
    )
    parser.add_argument(
        "--skb-slr-k-lim",
        default=False,
    ...
    parser.add_argument(
        "--cph-srv-ae", type=int, nargs="+", help="CoxPHSurvivalAnalysis alpha exp"
    )
    parser.add_argument(
        "--cph-srv-ae-min", type=int, help="CoxPHSurvivalAnalysis alpha exp min"
    )
    parser.add_argument(
        "--cph-srv-ae-max", type=int, help="CoxPHSurvivalAnalysis alpha exp max"
    )
    parser.add_argument(
        "--cph-srv-num-alphas",
        type=int,
    ...
    parser.add_argument(
        "--cph-srv-ties", type=str, default="efron", help="CoxPHSurvivalAnalysis ties"
    )
    parser.add_argument(
        "--cph-srv-n-iter", type=int, default=1000, help="CoxPHSurvivalAnalysis n_iter"
    )
    parser.add_argument(
        "--cph-srv-base-alpha",
        type=float,
    ...
    parser.add_argument(
        "--cnet-srv-l1r", type=float, nargs="+", help="CoxnetSurvivalAnalysis l1_ratio"
    )
    parser.add_argument(
        "--cnet-srv-l1r-min", type=float, help="CoxnetSurvivalAnalysis l1_ratio min"
    )
    parser.add_argument(
        "--cnet-srv-l1r-max", type=float, help="CoxnetSurvivalAnalysis l1_ratio max"
    )
    parser.add_argument(
        "--cnet-srv-l1r-step",
        type=float,
    ...
    parser.add_argument(
        "--cnet-srv-na", type=int, nargs="+", help="CoxnetSurvivalAnalysis n_alphas"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fsvm-srv-ae", type=int, nargs="+", help="FastSurvivalSVM alpha exp"
    )
    parser.add_argument(
        "--fsvm-srv-ae-min", type=int, help="FastSurvivalSVM alpha exp min"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        choices=["avltree", "direct-count", "PRSVM", "rbtree", "simple"],
        ...
        if args.parallel_backend == "multiprocessing":
            ...
            if "convergence" in args.filter_warnings:
                python_warnings.append(
                    ":".join(["ignore", "Optimization did not converge", "UserWarning"])
                )
                python_warnings.append(
                    ":".join(["ignore", "Optimization terminated early", "UserWarning"])
                )
            if "joblib" in args.filter_warnings:
                python_warnings.append(
                    ":".join(
    ...
    for cv_param, cv_param_values in cv_params.copy().items():
        ...
        if cv_param in (
            ...
            cv_params[cv_param[:-1]] = cv_params["cph_srv_am"] * 10.0 ** np.asarray(
                cv_param_values
            )
        elif cv_param == "cph_srv_ae_max":
            cv_param = "_".join(cv_param.split("_")[:-1])
            cv_param_v_min = (
                cv_params["cph_srv_am"] * 10.0 ** cv_params["{}_min".format(cv_param)]
            )
            cv_param_v_max = cv_params["cph_srv_am"] * 10.0**cv_param_values
            cv_params[cv_param[:-1]] = np.logspace(
                np.log10(cv_param_v_min),
                np.log10(cv_param_v_max),
                cv_params["cph_srv_num_alphas"],
            )
        elif cv_param == "fsvm_srv_ae":
            cv_params[cv_param[:-1]] = 2.0 ** np.asarray(cv_param_values)
    ...
    if cv_params["cnet_srv_l1r"] is not None and any(
        ...
        cv_params["cnet_srv_l1r"] = np.array(
            [cnet_srv_min_1lr]
            + [r for r in cv_params["cnet_srv_l1r"] if r >= cnet_srv_min_1lr]
        )
    ...
    pipe_config = {
        ...
        "CoxPHSurvivalAnalysis": {
            ...
            "param_grid": {"alpha": cv_params["cph_srv_a"]},
            "param_routing": ["feature_meta"],
        },
        "FastCoxPHSurvivalAnalysis": {
